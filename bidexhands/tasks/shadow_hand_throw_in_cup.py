...
class ShadowHandThrowInCup(BaseTask):
    """
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        block_start_pose.p = object_start_pose.p
        block_start_pose.r = gymapi.Quat().from_euler_zyx(1.57, 1.57, 0)
        block_start_pose.p.y -= 0.39
        ...
        for i in range(self.num_envs):
            ...
            if self.obs_type in ["point_cloud"]:
                ...
                self.gym.set_camera_location(camera_handle, env_ptr, gymapi.Vec3(0.25, -0.6, 1.2), gymapi.Vec3(-0.24, -0.6, 0.2))
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.reset_buf[:], self.reset_goal_buf[:], self.progress_buf[:], self.successes[:], self.consecutive_successes[:] = compute_hand_reward(
            self.rew_buf, self.reset_buf, self.reset_goal_buf, self.progress_buf, self.successes, self.consecutive_successes,
            self.max_episode_length, self.object_pos, self.object_rot, self.goal_pos, self.goal_rot, self.block_right_handle_pos, self.block_right_handle_rot, self.block_left_handle_pos, self.block_left_handle_rot,
            self.left_hand_pos, self.right_hand_pos, self.right_hand_ff_pos, self.right_hand_mf_pos, self.right_hand_rf_pos, self.right_hand_lf_pos, self.right_hand_th_pos, 
            self.left_hand_ff_pos, self.left_hand_mf_pos, self.left_hand_rf_pos, self.left_hand_lf_pos, self.left_hand_th_pos, 
            self.dist_reward_scale, self.rot_reward_scale, self.rot_eps, self.actions, self.action_penalty_scale,
            self.success_tolerance, self.reach_goal_bonus, self.fall_dist, self.fall_penalty,
...
def quaternion_to_rotation_matrix(quat):
    ...
    # 计算旋转矩阵的各个分量
    R = torch.zeros((quat.shape[0], 3, 3), dtype=torch.float32, device=quat.device)
    R[:, 0, 0] = 1 - 2 * (y * y + z * z)
    R[:, 0, 1] = 2 * (x * y - z * w)
    R[:, 0, 2] = 2 * (x * z + y * w)
    R[:, 1, 0] = 2 * (x * y + z * w)
    ...
    return R
def calculate_angle_between_z_axes(quat):
    # 将四元数转换为旋转矩阵
    ...
    # 世界坐标系的 z 轴方向向量
    world_z_axis = torch.tensor([0, 0, 1], dtype=torch.float32, device=quat.device).repeat(quat.shape[0], 1)
    
    # 计算夹角
    dot_product = torch.sum(object_z_axis * world_z_axis, dim=1)
    norms = torch.norm(object_z_axis, dim=1) * torch.norm(world_z_axis)
...
def compute_hand_reward(
    ...
    block_right_handle_pos, block_right_handle_rot, block_left_handle_pos, block_left_handle_rot,
    ...
    Args:
        ...
        block_right_handle_pos (tensor): The position of the right block(object cup) handle

        block_left_handle_pos (tensor): The position of the left block(cube) handle
    ...
    left_hand_finger_dist = (torch.norm(block_right_handle_pos - left_hand_ff_pos, p=2, dim=-1) + torch.norm(block_right_handle_pos - left_hand_mf_pos, p=2, dim=-1)
                            + torch.norm(block_right_handle_pos - left_hand_rf_pos, p=2, dim=-1) + torch.norm(block_right_handle_pos - left_hand_lf_pos, p=2, dim=-1) 
                            + torch.norm(block_right_handle_pos - left_hand_th_pos, p=2, dim=-1))
    # Orientation alignment for the cube in hand and goal cube
    # quat_diff = quat_mul(object_rot, quat_conjugate(target_rot))
    # rot_dist = 2.0 * torch.asin(torch.clamp(torch.norm(quat_diff[:, 0:3], p=2, dim=-1), max=1.0))

    # right_hand_dist_rew = torch.exp(-2 * right_hand_finger_dist)
    left_hand_finger_dist_rew = -torch.exp(-2 * left_hand_finger_dist)

    # rot_rew = 1.0/(torch.abs(rot_dist) + rot_eps) * rot_reward_scale

    action_penalty = -0.000*torch.sum(actions ** 2, dim=-1)
    ...
    near_rew = torch.exp(-0.2*(object_dist*25))
    ...
    left_right_height_rew = torch.where(
        left_right_height_diff >= 0.3,
        -(left_right_height_diff - 0.3)*2,
        left_right_height_rew
    )
    
    # reward = right_dist_rew + left_dist_rew + near_rew + rot_rew + left_right_height_rew + action_penalty + left_hand_finger_dist_rew
    reward = near_rew
    fall_flag1 = (block_left_handle_pos[:, 2] <= 0.2)
    fall_flag2 = (block_right_handle_pos[:, 2] <= 0.2)
    ...
    goal_resets = torch.where(object_dist < 0.2, torch.ones_like(reset_goal_buf), reset_goal_buf)
    # Find out which envs hit the goal and update successes count
    successes = torch.where(successes == 0, 
                    torch.where(object_dist < 0.2, torch.ones_like(successes), successes), successes)
    
    reward = torch.where(goal_resets == 1, reward + 250, reward)
    ...
    # print("right_hand_dist: ", right_hand_dist[0], "right_hand_rew: ", right_dist_rew[0])
    # print("left_hand_dist: ", left_hand_dist[0], "left_hand_rew: ", left_dist_rew[0])
    # print("object_dist: ", object_dist[0], "object_rew: ", near_rew[0])
    # print("rot_dist: ", rot_dist[0], "rot_rew: ", rot_rew[0])
    # print("left_right_height_diff: ", left_right_height_diff[0], "left_right_height_rew: ", left_right_height_rew[0])
